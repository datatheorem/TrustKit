...
pin=$(openssl x509  -pubkey -noout -in $CERT | openssl rsa -outform DER -pubin -in /dev/stdin 2>/dev/null | openssl dgst -sha256 -binary | openssl enc -base64)

echo "kTSKPublicKeyHashes: @[@\"$pin\"]"

# Generate the public key algorithm
publickey=$(openssl x509 -in $CERT -text -noout | grep 'Public Key')
